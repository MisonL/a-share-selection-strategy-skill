...
```
`backtest_buy_hold.py` 是 close-to-close 基线；`--cost-bps`/`--slippage-bps` 只按 round-trip bps 从 gross return 中扣减，可用 `--require-tradable-bars` 要求入场和退出日 `tradestatus=1`。候选表若含 `weight/notional/quantity/cash_reserved`，回测只透传这些字段；`portfolio_equity_curve.py` 生成等权资金曲线，`portfolio_overlap_report.py` 检查并发、同标的重叠，以及 `--max-gross-weight`、`--max-gross-notional`、`--max-cash-reserved` 门禁，但真实现金容量仍依赖可追溯的资金字段来源；这些脚本仍不判断涨跌停规则。
