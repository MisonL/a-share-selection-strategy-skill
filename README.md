...
```
`backtest_buy_hold.py` 是 close-to-close 基线；`--cost-bps`/`--slippage-bps` 只按 round-trip bps 从 gross return 中扣减。`portfolio_equity_curve.py` 只对已完成交易按信号日等权复利生成资金曲线，可用 `--min-final-equity`/`--max-drawdown-floor` 设失败门槛；二者都不判断涨跌停或停牌可交易性。
