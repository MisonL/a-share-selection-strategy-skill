...
```
`allocate_candidate_capital.py` 用信号日 close、现金预算和 lot size 生成可追溯 sizing 字段，模型为 `equal_cash_budget_lot_floor`。上例回测是未启用 `--require-tradable-bars` 的 close-to-close 基线；如需回测级可交易门禁，价格文件必须含 `tradestatus`，并显式加该参数。回测只透传资金字段；组合报告检查并发、同标的重叠、权重、名义金额和预留现金门禁；这些脚本仍不证明真实成交容量或判断涨跌停规则。当前真实门禁优先级以 `docs/reviews/REAL-SCENARIO-GATES-2026-05-30.md` 为准。
