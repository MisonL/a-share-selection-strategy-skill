...
```
`backtest_buy_hold.py` 是 close-to-close 基线；`--cost-bps`/`--slippage-bps` 只按 round-trip bps 从 gross return 中扣减，可用 `--require-tradable-bars` 要求入场和退出日 `tradestatus=1`。`portfolio_equity_curve.py` 按信号日等权复利生成资金曲线，可用 `--min-final-equity`/`--max-drawdown-floor` 设失败门槛；二者仍不判断涨跌停规则。
