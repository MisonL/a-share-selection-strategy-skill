...
本 Skill 提供三个可复用资源，位于 Skill 目录的 `scripts/` 下：
...
- `scripts/stock_selection_metrics.py`、`scripts/stock_selection_output.py`：评分脚本使用的指标和输出辅助函数。

使用方式：

```bash
python scripts/validate_ohlcv.py --input prices.csv
python scripts/score_candidates.py --input prices.csv --config scripts/example_config.json --output candidates.csv
python scripts/score_candidates.py --input prices_with_prediction.csv --config scripts/qsss_profile_config.json --output qsss_candidates.csv
```

脚本只处理本地文件，不联网取数，不调用券商接口，不生成交易指令。若输入是 Parquet 文件，需要当前 Python 环境已安装可用的 Parquet 引擎，例如 `pyarrow` 或 `fastparquet`。
...
- `close` 必须存在、非空且大于 0，否则跳过该股票并记录为 `invalid_data`。
- 历史行数不足 120 时跳过并记录为 `insufficient_data`。
- 技术指标和 ML 特征计算前，对 `close`、`volume`、`turn` 使用中位数加减 3 倍标准差裁剪异常值。
