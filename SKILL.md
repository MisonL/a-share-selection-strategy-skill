...
运行因子前先做以下处理。预设 `score_candidates.py` 采用严格输入校验：任一股票出现无效价格、负成交量、重复日期或历史不足时，脚本会直接失败并报告原因，避免把坏数据清洗成成功结果。若 Agent 自行实现批量服务，可以按股票跳过并记录失败分类，但必须让失败计数可见。

- `close` 必须存在、非空且大于 0；预设脚本会显式失败，批量服务可跳过该股票并记录为 `invalid_data`。
- 历史行数不足 120 时；预设脚本会显式失败，批量服务可跳过该股票并记录为 `insufficient_data`。
