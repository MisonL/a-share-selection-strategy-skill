...
| baostock | `date`、`code`、`open`、`high`、`low`、`close`、`volume`、`amount`、`turn` | 去掉 `sz.`/`sh.` 后写入 `symbol`，其余字段同名映射，补 `market=A-share` |
...
- `scripts/fetch_baostock_a_share.py`：可选 baostock A 股日线取数脚本，输出本地行情 CSV 和 metadata JSON。
...
- `scripts/generate_lightgbm_predictions.py`：可选 LightGBM 预测生成器，使用时间序列切分和训练集内 scaler，输出 `prediction_score`。
- `scripts/backtest_buy_hold.py`：可选 close-to-close buy-hold 基线回测，只用于门禁复验，不代表完整策略回测。
- `scripts/slice_prices_as_of.py`：按信号日截断本地行情文件，防止用未来行情生成候选后再回测。
...
`create_demo_data.py` 只依赖标准库，可以直接用裸 `python3` 运行；`validate_ohlcv.py`、`score_candidates.py`、`backtest_buy_hold.py` 和测试需要 `uv --with pandas --with numpy`，或使用已安装依赖的虚拟环境。没有 `uv` 时可运行 `python3 -m venv /tmp/stock-selection-skill-venv`，再用 `/tmp/stock-selection-skill-venv/bin/python -m pip install -r requirements.txt` 安装基础依赖。若输入是 Parquet 文件，还需要可用的 Parquet 引擎，例如 `pyarrow` 或 `fastparquet`；可安装 `requirements-parquet.txt`。运行真实 LightGBM 预测生成器需要额外安装 `requirements-ml.txt`。
...
评分、校验、切片、预测和回测脚本以本地文件为入口，不调用券商接口，不生成交易指令。`fetch_baostock_a_share.py` 是显式可选联网取数入口，只负责落地本地 gate 文件。`validate_ohlcv.py --config` 会在基础 OHLCV 校验之外检查 profile 专属字段。
...
- 可选 `generate_lightgbm_predictions.py` 是独立上游生成器。缺少 LightGBM 或 scikit-learn 会直接非 0；单个标的训练样本不足、目标只有单类或预测失败时，默认会跳过该标的并在 summary 中记录。真实门禁必须启用 `--fail-on-skipped`，或显式检查 `raw_symbols == predicted_symbols` 且 `skipped_symbols == 0`。它只把最新预测概率重复写入该标的所有行，目的是让现有评分脚本消费最新 `prediction_score`；不要把它解释成逐日历史预测序列。
...
回测边界：原 Web 回测只支持真实日线收盘价上的 `buy_hold` 基线。`backtest_buy_hold.py` 只匹配候选信号日的真实收盘价和未来第 N 个可用交易行收盘价；没有交易成本、滑点、涨跌停和不可交易状态时，不要把结果表述为完整策略回测。
