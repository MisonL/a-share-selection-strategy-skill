...
- 可选 `generate_lightgbm_predictions.py` 是独立上游生成器，会在缺少 LightGBM 或 scikit-learn、训练样本不足、目标只有单类、预测失败时非 0 退出。它只把最新预测概率重复写入该标的所有行，目的是让现有评分脚本消费最新 `prediction_score`；不要把它解释成逐日历史预测序列。
