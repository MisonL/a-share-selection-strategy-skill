...
- 生成结果继续通过 `validate_ohlcv.py --config scripts/prediction_profile_config.json`，再进入 `score_candidates.py --config scripts/prediction_profile_config.json`，最终 `scored_symbols=2`、`candidates=1`。
...
- 同一 SSE603 真实产物的 summary 字段只证明 40-symbol/6 信号日生成链路可审计；LightGBM 质量边界以 `docs/prediction-derived-profile.md` 的“LightGBM 质量边界”为准。
...
- 2-symbol、12-symbol 和 40-symbol baostock 证据只证明当前生成器能在有限真实行情样本上运行，并接入 `validate_ohlcv.py --config scripts/prediction_profile_config.json` 与 `score_candidates.py`；不证明模型质量已通过。
...
- 原始行情直接跑 `validate_ohlcv.py --config scripts/prediction_profile_config.json` 返回 1，错误为缺少 `prediction` 或 `prediction_score`，符合 prediction-derived 门禁预期。
...
- 生成结果通过 `validate_ohlcv.py --config scripts/prediction_profile_config.json`，再进入 prediction-derived 评分，输出 `scored_symbols=2`、`candidates=1`、`threshold_failures=min_prediction_score:1`。
...
- 低价超短意图已有通用技术评分 profile `scripts/ultra_short_low_price_config.json`，表达 `3 <= close <= 10`、成交量/成交额/换手硬阈值、排除 ST/停牌/一字板，以及更高短线爆发权重；它不是 prediction-derived，不使用也不伪造 LightGBM prediction。
