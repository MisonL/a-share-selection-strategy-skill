"""Universe filtering helpers for stock selection scoring."""
...
def apply_prefix_allow_filter(
    ...
    if not allow_regex:
        return frame
    before = frame["symbol"].nunique()
    result = frame[frame["symbol"].str.match(str(allow_regex))]
...
def apply_prefix_exclude_filter(
    ...
    if not exclude:
        return frame
    before = frame["symbol"].nunique()
    result = frame[~frame["symbol"].str.startswith(exclude)]
