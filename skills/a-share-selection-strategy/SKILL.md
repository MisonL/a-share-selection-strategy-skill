...
入口映射只在需要复制命令、确认依赖、字段映射或 helper 边界时展开。先用 [scripts/SCRIPTS.md](scripts/SCRIPTS.md) 区分稳定 CLI、取数入口、门禁回测入口和内部 helper；需要配置、依赖、字段映射或完整命令时再读 [references/script-reference.md](references/script-reference.md) 和 [instructions/runbook.md](instructions/runbook.md)。
...
| 全 A 严格任务 | [instructions/full-a-strict-workflow.md](instructions/full-a-strict-workflow.md) |
...
- 脚本入口机器注册表见 `configs/script_entrypoints.json`；它只用于审计根层 `.py` 分类，不做运行时 dispatch 或 CLI 合约替代。
