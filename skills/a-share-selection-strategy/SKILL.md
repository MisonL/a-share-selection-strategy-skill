...
- `allocate_candidate_capital.py`：可选候选资金分配脚本，按信号日 close、现金预算和 lot size 生成可追溯 sizing 字段；stdout 会披露 `cash_budget`、`lot_size`、`capital_model` 和 `claim_boundary=local_sizing_not_broker_order`。候选表已有 sizing/资金字段时默认拒绝，只有显式 `--overwrite-capital-fields` 才重算覆盖；输出不能解释为真实成交、券商订单或真实现金容量证明。
