...
5. 先读取 `summary.json` 或 stdout 中的 `execution_path`、`coverage_class`、`candidate_field_coverage`、`full_a_provenance_validation_status`（若存在）、`full_a_provenance_closure_eligible`、`full_a_provenance_as_of_date`、`full_a_provenance_final_filter_removed_symbol_count`、`full_a_provenance_output_cleanup_errors`、`full_market_claim_allowed`、`full_market_claim_boundary`、`selection_failed_reason`、`selection_failed_next_action`。
...
- `任务拓扑` 的首选动作只能引用 `skill_route=true` 的公开 CLI；这不是“29 个 public CLI 都是默认入口”，默认主入口仍是 `validate_ohlcv.py`、`score_candidates.py` 和 `run_today_a_share_selection.py`。新增默认路由前先更新注册表和文档一致性测试。
