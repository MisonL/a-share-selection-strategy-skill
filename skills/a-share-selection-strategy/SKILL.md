...
- `hong_kong_generic_config.json`：港股本地 OHLCV 通用技术评分剖面，要求 `market=HK`，支持 `00700`、`HK.00700`、`00700.HK` 等 1 到 5 位港股代码展示；不使用 prediction-derived/LightGBM，不证明港交所日历、真实成交或收益。
...
- `score_candidates.py`：读取本地行情文件并输出候选股 CSV；`--output` 和 `--diagnostics-output` 只支持 CSV 后缀，传 `.parquet` 或 `.pq` 会失败以避免假 Parquet 产物。可用 `--spot-input` 合并实时价、涨跌幅、行业和成交额展示字段，但这些字段不参与核心评分。spot symbol 列支持 `symbol/code/code_id/stock_code/ticker/Ticker`，`sh.600000`、`600000.SH`、`sz.000001`、`000001.SZ` 会归一化为六位代码后匹配；港股本地输入可用 `market=HK` 配合 `hong_kong_generic_config.json`，板块展示按港股代码推导为“港股主板”或“港股 GEM”。
- `run_today_a_share_selection.py`：总控 CLI，串联可选实时快照、可选仓库内历史取数、`validate_ohlcv.py`、`score_candidates.py` 和诊断输出，标准输出 `run_manifest.json`、`summary.json`、`report.html`、`candidates.csv`、`diagnostics.csv`，并使用 CSV 中间产物；当前不支持严格全 Parquet 输出，也不证明实时全市场扫描完成。历史源支持 `akshare`、`akshare_hk_daily`、`baostock`、`zzshare` 和 `yfinance`；`akshare_hk_daily` 使用 akshare `stock_hk_daily` 拉取港股日线 OHLCV，仍不证明港交所日历、实时行情、可交易性或长期稳定性；`yfinance` 会按所选 config 的 `universe.market` 写入 market 标签，港股 config 下可把 `00700`、`HK.00700` 或 `00700.HK` 映射为 Yahoo ticker，但 metadata 仍声明 `market_label_only=true`，不证明港交所源、交易日历、实时行情或长期稳定性。`report.html` 是人类可读展示层，支持 `--html-report-language auto|zh|en` 和浏览器内中英文切换，可用 `--no-html-report` 关闭，不能替代 JSON/CSV 机器事实。
...
- `fetch_akshare_hk_daily.py`：可选 akshare 港股日线取数脚本，使用 `stock_hk_daily` 输出本地 OHLCV CSV 和 metadata JSON；输出 `market=HK`，但 `real_market_data=unknown` 和 `source_claim_boundary=akshare_stock_hk_daily_not_exchange_calendar_or_tradability_proof` 表示它不证明港交所交易日历、实时行情、真实可交易性或长期稳定性。
