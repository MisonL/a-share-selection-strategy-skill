...
- 2026-06-08 已按上述方向推进一次 current gates closeout，详见 `skills/a-share-selection-strategy/evidence/reviews/CURRENT-GATES-CLOSEOUT-2026-06-08.md`。P1 固定 40-symbol 组合容量 artifact validator 通过；P2 核心控制字段探针通过但直接涨跌停字段和交易状态候选字段仍不可用，完整控制字段探针因 `volume,amount` provider error 返回 3；P3 三轮外部源观察返回 3，`passed_runs=3/9`，akshare 和 yfinance 均未通过，baostock 3/3 通过。该 closeout 把可执行项推进到证据边界，但不能把真实涨跌停规则、长期稳定性、全市场收益或真实券商成交写成已证明。
