...
def scoring_method_key(summary: dict[str, Any]) -> str:
    ...
    return "prediction_method_value" if summary.get("prediction_mode") else "generic_method_value"
...
def prediction_status_key(summary: dict[str, Any]) -> str:
    ...
    if prediction_scoring_consumed_input(summary) or prediction_scoring_completed(summary):
...
def prediction_columns_missing(summary: dict[str, Any]) -> bool:
    return bool(missing_prediction_groups(summary) or summary.get("missing_prediction_requirement"))
...
def run_failed(summary: dict[str, Any]) -> bool:
    return str(summary.get("status", "")) == "failed" or bool(summary.get("failed_steps", []))
