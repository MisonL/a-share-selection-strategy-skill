...
import zzshare_a_share_data as data
...
def apply_quality_policy(
    ...
    *,
    ...
    metadata["symbols"] = [
        symbol_metadata_for_frame(symbol, result, metadata_symbol_pages(metadata, symbol))
...
def metadata_symbol_pages(metadata: dict[str, Any], symbol: str) -> tuple[int, bool]:
    for item in metadata.get("symbols", []):
        if str(item.get("symbol", "")) == symbol:
            return int(item.get("pages_used", 0)), bool(item.get("possibly_truncated", False))
    return 0, False
...
def symbol_metadata_for_frame(symbol: str, frame: Any, pages: tuple[int, bool]) -> dict[str, Any]:
