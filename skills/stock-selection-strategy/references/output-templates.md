...
- 复用脚本函数前，必须把仓库的 `skills/stock-selection-strategy/scripts/` 加入 `PYTHONPATH` 或 `sys.path`。
- 不要把 `from skills/stock-selection-strategy/scripts/score_candidates import ...` 当成稳定 API；它可能在 import 阶段成功，但调用时因内部顶层模块路径缺失而失败。
- `python3 -S skills/stock-selection-strategy/scripts/<name>.py --help` 依赖轻量化门禁只覆盖带 `argparse`、`main()` 或 `__main__` 的 CLI 入口；`stock_selection_*.py`、`lightgbm_prediction_summary.py` 等 helper/import 模块没有帮助界面承诺，顶层 pandas/numpy import 失败不应写成用户入口缺口。
- 使用 `skills/stock-selection-strategy/scripts/*.py` 全量扫描时，必须先把真实 CLI 入口和 helper/import 模块分开；不能用 glob 扫描里的 helper 失败覆盖真实 CLI 入口的逐项验证结果。
...
- `score_candidates.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json` 报缺字段、`code=bad_input` 或 `output_written=false` 时，是输入门禁失败，不是成功 0 候选。
