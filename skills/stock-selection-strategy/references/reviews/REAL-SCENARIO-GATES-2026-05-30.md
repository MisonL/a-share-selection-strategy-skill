...
历史 stdout 和字段名按当时运行结果原样保留。若与当前 README、SKILL 或 `skills/stock-selection-strategy/references/output-templates.md` 的优先字段不同，当前报告应优先使用新文档中的中性字段，历史报告只作为当时证据。
...
- 本轮新增 `skills/stock-selection-strategy/scripts/generate_lightgbm_predictions.py`，独立生成 `prediction_score`，并在缺少 `lightgbm` 或 `scikit-learn` 时非 0 退出。
...
- 本地真实依赖复验已通过: `uv run --with-requirements skills/stock-selection-strategy/requirements-ml.txt python skills/stock-selection-strategy/scripts/generate_lightgbm_predictions.py --input /tmp/stock-selection-ml-local/prices.csv --output /tmp/stock-selection-ml-local/prices_generated_prediction.csv` 返回 0。
- 生成结果继续通过 `validate_ohlcv.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json`，再进入 `score_candidates.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json`，最终 `scored_symbols=2`、`candidates=1`。
...
- 同一 SSE603 真实产物的 summary 字段只证明 40-symbol/6 信号日生成链路可审计；LightGBM 质量边界以 `skills/stock-selection-strategy/references/prediction-derived-profile.md` 的“LightGBM 质量边界”为准。
...
- 2-symbol、12-symbol 和 40-symbol baostock 证据只证明当前生成器能在有限真实行情样本上运行，并接入 `validate_ohlcv.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json` 与 `score_candidates.py`；不证明模型质量已通过。
...
- 本轮新增 `skills/stock-selection-strategy/scripts/backtest_buy_hold.py`。
...
- 原始行情直接跑 `validate_ohlcv.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json` 返回 1，错误为缺少 `prediction` 或 `prediction_score`，符合 prediction-derived 门禁预期。
...
- 生成结果通过 `validate_ohlcv.py --config skills/stock-selection-strategy/scripts/prediction_profile_config.json`，再进入 prediction-derived 评分，输出 `scored_symbols=2`、`candidates=1`、`threshold_failures=min_prediction_score:1`。
...
- 低价超短意图已有通用技术评分 profile `skills/stock-selection-strategy/scripts/ultra_short_low_price_config.json`，表达 `3 <= close <= 10`、成交量/成交额/换手硬阈值、排除 ST/停牌/一字板，以及更高短线爆发权重；它不是 prediction-derived，不使用也不伪造 LightGBM prediction。
